...
def generar_mapa_base_folium(df_regiones):
    ...
    # Crear mapa base con un estilo más atractivo y atribuciones
    mapa = folium.Map(
...
def generar_mapa_con_ruta_folium(df_regiones, ruta, distancias=None):
    ...
    # Añadir líneas para cada segmento de la ruta con colores graduales
    colores = ['#FF6B6B', '#FF8E8E', '#FFB1B1', '#FFD4D4', '#FFF7F7']
    ...
    for i in range(len(ruta) - 1):
        ...
        # Calcular el color basado en la posición en la ruta (inicio a fin)
        color_idx = min(i, len(colores) - 1)
        color = colores[color_idx] if i < len(colores) else colores[-1]
...
def convertir_pillow_a_qpixmap(image):
    ...
    Args:
        image: Imagen de Pillow a convertir
        
    Returns:
        QPixmap: Imagen convertida para ser mostrada en Qt
    """
    byte_arr = io.BytesIO()
